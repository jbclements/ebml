...
(require (only-in "reader.rkt" Element))

(provide ebml->bytes ebml-write)

;; given a list of ebml-elements, write them to a byte string
(: ebml->bytes : (Listof Element) -> Bytes)
...
(: ebml-write (case-> ((Listof Element) -> Any)
                      ((Listof Element) Output-Port -> Any)))
(define (ebml-write elements [port (current-output-port)])
  (for: ([element : Element elements])
    (ebml-element-write element port)))

;; write a single element to a port
(: ebml-element-write (case-> (Element -> Any)
                              (Element Output-Port -> Any)))
(define (ebml-element-write element [port (current-output-port)])
  (match-define (list id #{data : (U Bytes (Listof Element))}) element)
...
(: ebml-header-id-write : Natural Output-Port -> Any)
...
;; representation of the id to the port.
(: ebml-data-length-write : Natural Output-Port -> Any)
...
(: ebml-data-length : (U Bytes (Listof Element)) -> Natural)
(define (ebml-data-length data)
  (match data
    [(? bytes? b) (bytes-length b)]
    [(? list? elts)
     (for/sum: : Natural ([elt : Element elts]) (ebml-element-length elt))]))

(: header-encoded-length : Natural -> Byte)
...
(: data-length-encoded-length : Natural -> Byte)
...
(module+ test
  ...
  (: writer->bytes : (Output-Port -> Any) -> Bytes)
